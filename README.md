...
5. Wait until it finishes, which usually takes a very long time. 

There will be likely that Google Colab will automatically close before we 
finish our training session. That's why in the model we've already implemented 
the function to save parameters in each checkpoint on Google Drive to help the 
